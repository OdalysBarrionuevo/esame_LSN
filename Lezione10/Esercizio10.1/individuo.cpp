...
void Individuo::Fill(Random& rnd){ //Giusto non modificarlo
...
void Individuo::Check(){ //Hai tolto +1 da visited
...
void Individuo::Swap(Random& rnd){
    ...
    for (int i = 1; i < size_individuo; i++) {
        ...
        //swap(I.GetValue(i), I.GetValue(randomIndex));
        int temp = a[i];
        a[i] = a[randomIndex];
        a[randomIndex] = temp;
    }
...
vector <int> Individuo::Get_listCities(){
    vector <int> a;
    a = individuo;
    return a;
};
