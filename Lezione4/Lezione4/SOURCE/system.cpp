...
void System ::move(int i) { // Propose a MC move for particle i
  if (_sim_type == 3) {     // Gibbs sampler for Ising
    ...
    _naccepted ++;
    double JSSH = _J*(_particle(this->pbc(i - 1)).getspin() +
             _particle(this->pbc(i + 1)).getspin())+_H;
    if(_rnd.Rannyu()< 1./(1+exp(-2*_beta*JSSH)))
  ...
  if (input.is_open()) {
    while (!input.eof()) {
      ...
      if (property == "POTENTIAL_ENERGY") {
        ...
        index_property++;
        _vtail = 8.*M_PI*_rho/double(9.*pow(_r_cut,9))-8.*M_PI*_rho/double(3.*pow(_r_cut,3)); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32.*M_PI*_rho*(1./double(9.*pow(_r_cut,9))-1./double(6.*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
...
void System ::read_configuration() {
  ...
  if (_restart and _sim_type > 1) {
    cout << "Reading from previous spin configuration" << endl;
    int spin;
...
void System ::measure() { // Measure properties
  ...
  double tenergy_temp2_av = 0.0;
  double spin_temp = 0.0;
      ...
      for (int j = i + 1; j < _npart; j++) {
        ...
        if (dr < _r_cut) {
          ...
          if (_measure_pressure)
              virial += 48.*(1./pow(dr,12)-0.5*(1./pow(dr,6))) + _ptail;
        }
      }
    }
  }
  ...
  if (_measure_tenergy) {
    if (_sim_type < 2)
      ...
      tenergy_temp2_av = tenergy_temp * tenergy_temp;
  ...
  if (_measure_pressure and _measure_temp){ //ho aggiunto "and _mesure_temp"
      virial = 1./(3.0*_volume)*virial/double(_npart);
      _measurement(_index_pressure) = _rho*_measurement(_index_temp)+virial;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if (_measure_magnet) { //MAGNITIZATION (=0 con h=0)
    for (int i = 0; i < _npart; i++)
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
    if (_measure_cv and _measure_tenergy){ //SPECIFIC HEAT h=0
        _measurement(_index_cv) = tenergy_temp2_av;
    }
  
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if(_measure_chi){ // SUSCEPTIBILITY h=0
    for (int i = 0; i < _npart; i++)
      spin_temp += _particle(i).getspin();
    _measurement(_index_chi) = _beta*pow(spin_temp,2)/double(_npart);
  }
  //GOFR///////////////////////////////////////////////////////////////////////
  if(_measure_gofr){
      for (int i = 0; i < _n_bins; i++) {
          double r_low = i * _bin_size;
          double r_up = r_low + _bin_size;
          double deltaV = (4.0 / 3.0) * M_PI * (pow(r_up, 3) - pow(r_low, 3));
          _measurement(i) /= (_rho * _npart * deltaV);
      }
  }
...
void System ::averages(int blk) {
  ...
  _average = _block_av / double(_nsteps); //tutto mediato su _npart e _nsteps
  //Scommentare sotto per calcolare il calore specifico
  //_average(_index_tenergy) = _average(_index_tenergy)*double(_npart);
  //_average(_index_cv) = _beta*_beta*(_average(_index_cv) - pow(_average(_index_tenergy), 2.))/double(_npart);
  //_average(_index_tenergy) = _average(_index_tenergy)/double(_npart);
  ...
  if (_measure_penergy) {
    ...
    coutf << setw(16) << blk << setw(16) << average << setw(16)
          << sum_average / double(blk) << setw(16)
  ...
  if (_measure_tenergy) {
    ...
    coutf << setw(16) << blk << setw(16) << setprecision(7) << average << setw(16)
          << sum_average / double(blk) << setw(16)
          << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  ...
  // TO BE FIXED IN EXERCISE 7
    if (_measure_gofr) {
        if( blk == _nblocks){
            ...
            for (int i = 0; i < _n_bins; i++) {
                // Calcola valori medi per ciascun bin
                average = _average[i];
                sum_average = _global_av[i];
                sum_ave2 = _global_av2[i];
                
                // Scrittura dei risultati per ogni bin
                coutf << setw(16) << i
                << setw(16) << average
